...
public class Warn {
    ...
    @Autowired
    private com.ggbond.defectdetection.service.LogRecordService logRecordService;
        ...
        return ()->{
            ...
            while(true){
                ...
                if(DataModule.getDefectiveRate()>defectiveRateWarningLine&&(now-lastWarn1>warningInterval*1000)){
                    ...
                    // 记录预警审计日志
                    try {
                        logRecordService.recordWarn("异常告警", "发现预警", "系统预警", "发现预警");
                    } catch (Exception ex) {
                        log.warn("记录预警日志失败: {}", ex.getMessage());
                    }
